...
if (numPointsProcessed >= totalInputPoints)  {
   ...
   maxOutputEntries = numPointsProcessed * 3*3*500; // 3 values per triangle, and there will be about 2 times as many triangles as input points
   ptrToOutputList = (WORDS*)malloc((maxOutputEntries + 1) * sizeof(WORDS));
   currenOutputIndex = 0;
   return 0;
}
   ...
   if (v0 >= 0 && v1 >= 0 && v2 >= 0 && v3 >= 0)  {
      ...
      if (triangleDirection)  {
         ...
         if ( ((triangleDirection > 0) && !isCW)  ||   // if user wants CW triangles, but it is not CW
            ...
            ptrToOutputList[currenOutputIndex++] = (WORDS)v2;
            ptrToOutputList[currenOutputIndex++] = (WORDS)v1;
            ptrToOutputList[currenOutputIndex++] = (WORDS)v0;
            return;
         }
      }
      ptrToOutputList[currenOutputIndex++] = (WORDS)v0;
      ...
      ptrToOutputList[currenOutputIndex++] = (WORDS)v2;
      if(pdim == 3)
        ptrToOutputList[currenOutputIndex++] = (WORDS)v3;//********for 3D
...
// ------------------------------------------------------
WORDS *BuildTriangleIndexList (void *pointList, float factor, int numberOfInputPoints, int numDimensions, int clockwise, int *numTriangleVertices ) {
